...
# 2. JS exceptions — baseline is 0, so any occurrence is worth investigating.
resource "azurerm_monitor_scheduled_query_rules_alert_v2" "js_exceptions" {
  ...
  enabled               = true
  auto_mitigation_enabled = true
  description           = "One or more client-side JS exceptions in the last 15 minutes (baseline is 0)."
  display_name          = "JS exceptions: any occurrence"
...
# memory: 128Mi) — not the observed baseline (~1-4% of the much smaller
# request) — because limit-relative headroom is what predicts real
# throttling/OOMKill risk regardless of current traffic. See ADR 0014.
